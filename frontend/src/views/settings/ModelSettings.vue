<template>
  <div class="model-settings">
    ...
    <!-- ChatBot 厂商配置 -->
    <div class="chatbot-config-section" style="margin-bottom: 24px;">
      <div class="category-header" style="margin-bottom: 12px;">
        <h3 style="margin: 0; font-size: 16px; font-weight: 600;">ChatBot 厂商配置</h3>
      </div>
      <!-- 凭证失效警告 -->
      <div v-if="chatBotNeedsReinit" style="margin-bottom: 12px; background: #fff7ed; border: 1px solid #fed7aa; border-left: 3px solid #f97316; border-radius: 6px; padding: 12px 16px; display: flex; align-items: flex-start; gap: 10px;">
        ...
        <div style="font-size: 13px; color: #9a3412; line-height: 1.5;">
          <strong>ChatBot 凭证已失效</strong><br />
          {{ chatBotReinitReason || '服务重启后加密密钥已变更，已保存的凭证无法解密。' }}请重新填写 APPID 和 APPSECRET 并点击"保存并初始化"以恢复服务。
        </div>
      </div>
      <div class="chatbot-config-card" style="background: var(--td-bg-color-container); border: 1px solid var(--td-component-stroke); border-radius: 8px; padding: 20px;">
        <div style="display: flex; align-items: flex-end; gap: 16px; flex-wrap: wrap;">
          <t-form-item label="APPID" style="margin-bottom: 0;">
            <t-input v-model="chatBotForm.appId" placeholder="请输入 APPID" style="width: 240px;" />
          </t-form-item>
          <t-form-item label="APPSECRET" style="margin-bottom: 0;">
            <t-input
              v-model="chatBotForm.appSecret"
              type="password"
...
import { listModels, createModel, updateModel as updateModelAPI, deleteModel as deleteModelAPI, initializeChatBot, getChatBotStatus, type ModelConfig } from '@/api/model'
...
// ChatBot 配置
const chatBotForm = ref({ appId: '', appSecret: '' })
const chatBotLoading = ref(false)
const chatBotNeedsReinit = ref(false)
const chatBotReinitReason = ref('')

const handleChatBotInit = async () => {
  if (!chatBotForm.value.appId || !chatBotForm.value.appSecret) {
  ...
  try {
    const result = await initializeChatBot({
      app_id: chatBotForm.value.appId,
      app_secret: chatBotForm.value.appSecret,
    })
    ...
    chatBotForm.value.appId = ''
    chatBotForm.value.appSecret = ''
    chatBotNeedsReinit.value = false
    chatBotReinitReason.value = ''
    await loadModels()
  } catch (err: any) {
    MessagePlugin.error(err?.message || 'ChatBot 初始化失败')
  } finally {
...
const loadModels = async () => {
  ...
  try {
    ...
    chatBotNeedsReinit.value = status.needs_reinit
    chatBotReinitReason.value = status.reason || ''
  } catch (error: any) {
