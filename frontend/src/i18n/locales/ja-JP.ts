export default {
  ...
  knowledgeBase: {
    ...
    tagSearchPlaceholder: '入力してタグを絞り込み',
  ...
  settings: {
    ...
    sandbox: {
      ...
      skillRemoveWaiting: 'イメージからのアンインストールを開始しました。進捗を待機中…',
      ...
      skillTranscriptWaiting: 'インストールを開始しました。処理ログを待機中…',
      skillFiles: 'ファイルを表示',
  ...
  auth: {
    ...
    redirectingToOIDC: 'IDプロバイダにリダイレクト中...',
    ...
    loginSuccessContent: 'おかえりなさい。{name}に入りました',
    loggingIn: 'ログイン中...',
    register: '登録',
    registering: '登録中...',
  ...
  input: {
    ...
    sharedAgentNotReadyDetail: '共有エージェント「{agentName}」は準備できていません（不足: {reasons}）。共有元の共有スペースの管理者に設定の完了を依頼してください。',
    webSearch: {
      toggleOn: 'Web検索を有効化',
      toggleOff: 'Web検索を無効化',
  ...
  knowledgeList: {
    ...
    uploadProgress: {
      ...
      completedDetail: '{total}件のファイルをすべてアップロードしました。解析ステータスを表示するため一覧を更新中...',
      refreshing: '解析ステータスを表示するため一覧を更新中...',
  ...
  knowledgeEditor: {
    ...
    faqImport: {
      ...
      previewMore: '他に{count}件は表示されていません',
  ...
  chat: {
    ...
    memoryHint: 'これらは、この回答が参照した長期メモリです。削除すると以降は使用されません。',
    ...
    connectingModelAndGeneratingAnswer: 'モデルに接続して回答を生成中…',
    modelStillResponding: 'モデルの応答に通常より時間がかかっています。引き続き待機中…',
    deepThoughtCompleted: '深い思考が完了しました',
    deepThoughtAlt: '深い思考が完了',
    referencesTitle: '関連する{count}件の内容を参照',
  ...
  tenant: {
    ...
    switchSuccessContent: '{name}に移動しました',
    ...
    create: {
      ...
      descriptionPlaceholder: 'このワークスペースの用途を簡単に説明',
      submit: '作成',
      cancel: 'キャンセル',
    ...
    api: {
      title: 'API情報',
      docLabel: 'APIドキュメント',
      docDescription: '完全なAPIドキュメントとサンプルはこちら：',
      openDoc: 'ドキュメントを開く',
      userIdLabel: 'ユーザID',
    ...
    globalSettings: {
      ...
      keyDescriptions: {
        ...
        asynq: {
          ...
          postprocess_concurrency: '軽量な解析の最終処理とエンリッチメントの振り分け専用の、プロセスごとの並列実行数です。最小値は1で、サービスの再起動が必要です。',
      ...
      createUser: {
        label: 'ユーザを作成',
        description: 'ローカルユーザアカウントを新規に作成します。プラットフォームのデフォルトのワークスペースモードが「個人ワークスペースを作成」の場合、セルフサービスで登録したユーザと同様に個人ワークスペースが作成されます。',
        action: 'ユーザを作成',
  ...
  platform: {
    ...
    description: 'RAG検索、エージェント推論、Wikiナレッジベースにより、ドキュメントが本当に理解され、活用されます',
    rag: 'RAG拡張生成',
    agent: 'ReActエージェント',
    wiki: 'Wikiナレッジベース',
  ...
  modelSettings: {
    ...
    usage: {
      ...
      truncated: '{total}件中、最初の{shown}件を表示',
      bindings: {
  ...
  organization: {
    ...
    rbac: {
      ...
      cannotCreate: '現在のワークスペースでのロールでは権限が足りないため、共有スペースを作成できません',
      cannotJoin: '現在のワークスペースでのロールでは権限が足りないため、共有スペースへの参加や参加申請ができません',
      cannotManage: '現在のワークスペースでのロールでは権限が足りないため、この共有スペースを管理できません'
    },
    join: {
      title: '共有スペースに参加',
      joining: '共有スペースに参加中...',
    ...
    settings: {
      ...
      removeShareFromOrg: '共有スペースから共有を解除',
      removeShareConfirm: '「{name}」のこの共有スペースへの共有を解除しますか？メンバーはこのナレッジベースにアクセスできなくなります。',
      removeAgentShareConfirm: '「{name}」のこの共有スペースへの共有を解除しますか？メンバーはこのエージェントにアクセスできなくなります。',
  ...
  faqManager: {
    import: {
      ...
      failed: '失敗',
      skipped: 'スキップ',
      progressHint: 'FAQデータを検証してインポート中...',
  ...
  tenantMember: {
    ...
    loading: 'メンバーを読み込み中…',
    retry: '再試行',
    empty: 'まだメンバーがいません。まずは招待して始めましょう。',
