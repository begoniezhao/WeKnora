...
> **通过 WeKnora 注册的 mcp-api-requester MCP 工具，你可以直接调用 OpenMAIC API**（HTTP POST/GET 请求）。
> 该工具在 WeKnora agent 中注册为 `mcp_{service_name}_{tool_name}` 格式（如 `mcp_mcp_api_requester_make_request`）。
> 不再需要向用户提供 curl 命令——直接在后台调用 API 并交付结果。
> 如果 MCP 工具未配置或不可用，则降级为展示 curl 命令让用户手动执行。
...
| 本地模式 | `http://host.docker.internal:3000`（WeKnora 在容器内）或 `http://localhost:3000`（本地直接运行） | 无认证（本地自部署） | 用户自行部署了 OpenMAIC 实例 |
...
- 如果找不到 HTTP 请求类 MCP 工具，则降级为 curl 方案
...
| 本地模式 | `http://host.docker.internal:3000` 或 `http://localhost:3000` | 无 |
...
**降级方式**：如果 MCP 工具不可用，向用户展示 curl 命令：
...
**本地模式**：
```bash
curl -X POST <BASE_URL>/api/generate-classroom \
  -H "Content-Type: application/json" \
  -d '{"requirement": "..."}'
```
...
| Provider 配置错误 | 模型/Provider/认证问题 | 引导用户检查 `.env.local` 或 `server-providers.yml` |
...
4. 如果 MCP 工具不可用，依次提供对应的 curl 命令
5. 汇总返回所有 Classroom URL

## 注意事项

- 脚本在 Docker 沙箱中执行，**沙箱默认禁用网络访问**
- **通过 WeKnora MCP 工具，你可以直接调用 OpenMAIC API**——不再需要用户提供 curl 命令
- MCP 工具名称格式为 `mcp_{service_name}_{tool_name}`，根据描述识别 HTTP 请求工具
- 如果 MCP 工具未启用或不可用，降级为展示 curl 命令让用户手动执行
